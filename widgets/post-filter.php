...
class Post_Filter extends \Elementor\Widget_Base {
    ...
    */
    public function get_title() {
        return __( 'Post Filter Bar', 'yx-super-cat' );
    }
    ...
    */
    protected function _register_controls() {
        ...
        $this->add_control(
            'all_text',
            [
                'label' => __( 'Text to show for <b>Show All</b>', 'yx-super-cat' ),
                'type' => \Elementor\Controls_Manager::TEXT,
                'default' => "all"
            ]
        );
    ...
    */
    protected function render() {
        ...
        $placeholder = '<li
        class="yx-super-cat-post-filter elementor-portfolio__filter elementor-active"
        data-term=""
        data-container="'.$filtererId.'"
        data-posts="'.$settings['post_id'].'">
        '. __($settings['all_text'], 'yx-super-cat').'
        </li>';
        foreach ($terms as $k => $v) {

          	$slug = (preg_match("/\p{Hebrew}/u", urldecode($v->slug))?$v->term_id : $v->slug);
            $li[] = '<li
            class="yx-super-cat-post-filter elementor-portfolio__filter"
